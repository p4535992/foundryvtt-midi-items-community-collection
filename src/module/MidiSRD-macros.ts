...
export class MidiMacros {
	...
	static templateCreation(templateData, actor) {
		const doc = new CONFIG.MeasuredTemplate.documentClass(templateData, { parent: <Scene>canvas.scene });
		//@ts-ignore
	...
	static async deleteTemplates(flagName, actor) {
		const removeTemplates = <MeasuredTemplate[]>(
			canvas.templates?.placeables.filter(
				(i) => i.data.flags["midi-items-community"]?.[flagName]?.ActorId === actor.id
			)
		);
	...
	static async warpgateCrosshairs(source, maxRange, name, icon, tokenData, snap) {
		...
		const checkDistance = async (crosshairs) => {
			while (crosshairs.inFlight) {
				...
				const distance = <number>canvas.grid?.measureDistances([{ ray }], { gridSpaces: true })[0];
	...
	static async animateDead(args) {
		...
		for (let i = 0; i < cycles; i++) {
			...
			const index = <any>pack.index.find((i) => i.name === dialog.actor.name);
			const compendium = <any>await pack.getDocument(index._id);
			const updates = {
	...
	static async arcaneHand(args, texture) {
		...
		if (args[0] === "on") {
			...
			const { x, y } = await MidiMacros.warpgateCrosshairs(
				token,
				120,
				"Arcane Hand",
				texture,
			...
			const range = <Promise<any>>canvas.scene?.createEmbeddedDocuments("MeasuredTemplate", [
				{
					...
					y: casterToken.y + <number>canvas.grid?.size / 2,
					direction: 0,
					distance: 60,
					borderColor: "#FF0000",
	...
	static async contagion(args) {
		...
		if (args[0] === "each") {
			...
			if (contagion.label === "Contagion") Contagion();
		}
		...
		if (args[0] === "on") {
			...
			Hooks.once("createMeasuredTemplate", async (template) => {
				const radius =
					...
					(template.data.distance / <number>canvas.grid?.grid?.options.dimensions?.distance);
	...
	static async removeWalls(actor: Actor) {
		const darkWalls = <Wall[]>(
            //@ts-ignore
			canvas.walls?.placeables.filter((w) => w.data.flags["midi-items-community"]?.Darkness?.ActorId === actor.id)
		);
	...
	static async fleshToStone(args) {
		...
		if (args[0] === "off") {
			...
			await ChatMessage.create({
				content:
					"Flesh to stone ends, if concentration was maintained for the entire duration,the creature is turned to stone until the effect is removed. ",
			});
		}
		...
		if (token) {
			if (args[0] === "on") {
				await ChatMessage.create({
					...
					whisper: [<StoredDocument<User>>game.user],
				});
...
`;
	...
	static async regenerate(args) {
		...
		function roundCount(currentTime, updateInterval, applyTime, expireTime) {
			...
			let roundCount = 0;
			if (lRound >= fRound) roundCount = (lRound - fRound) / 6 + 1;
